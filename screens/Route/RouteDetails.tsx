...
export default function RouteDetails({ navigation, route }: any) {
  ...
  if (isLoading)
    return (
      <Flex flex={1} justify="center" align="center" bg={"black"}>
        <Spinner color={colors.green} size="lg" />
      </Flex>
    );

  return (
    <VStack flex={1} p={5} space={4} bg={"black"}>
      <IconButton
              ...
              <Flex direction="row" justify="space-between" align="center">
                ...
                <IconButton
                  icon={
                    <Entypo
                      name={
                        selectedMatch == idx ? "chevron-up" : "chevron-down"
                      }
                      size={30}
                      color={colors.green}
                    />
                  ...
                  variant="unstyled"
                  onPress={() => {
                    if (selectedMatch == idx) setSelectedMatch(undefined);
                    else setSelectedMatch(idx);
                  }}
                />
              </Flex>
              {selectedMatch == idx && (
                <Flex>
                  ...
                  >
                    Usuário:{" "}
                    <Text color={colors.green}>{match.user_login}</Text>
                  </Text>
                  <Flex direction="row" justify="space-around" mt={8}>
                    <IconButton
                      ...
                      onPress={() => {
                        ...
                        Linking.openURL(
                          `https://wa.me/${match.user_phone}?text=Olá, ${match.user_login}! Vi no MyWay que você compartilha uma de minhas rotas e gostaria de saber se posso te acompanhar.\n\n${mapsRoute}`
                        );
                      }}
                    />
                    <IconButton
